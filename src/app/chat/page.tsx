...
import { htmlToMarkdown, copyToClipboard } from '@/lib/html-to-markdown'
...
export default function ChatPage() {
  ...
  useEffect(() => {
    const loadConversations = async () => {
      try {
        ...
        if (conversationsData.length && !currentConversationId) {
  ...
  useEffect(() => {
    if (currentConversationId) {
      const loadMessages = async () => {
        try {
          const res = await fetch(`/api/chat/messages?conversation_id=${currentConversationId}`, { method: 'GET' })
          const data = await res.json()
  ...
  const ensureConversation = async (messageContent?: string): Promise<string> => {
    ...
    // Only create new conversation when needed
    ...
    setFilteredConversations(prev => [data.conversation, ...prev])
  ...
  const handleCopyAsMarkdown = async (messageId: string, htmlContent: string) => {
    try {
  ...
  return (
    <div className="flex" style={{ minHeight: '100vh', background: 'var(--bg)' }}>
        ...
        <div className="w-80 flex flex-col" style={{ background: 'var(--surface)', borderRight: '1px solid var(--border)' }}>
          <div className="p-4" style={{ borderBottom: '1px solid var(--border)' }}>
            ...
            <div className="relative">
              ...
              />
              <span className="absolute left-2 top-1/2 transform -translate-y-1/2 text-muted">
                🔍
              </span>
              {historySearch && (
                <button
          ...
          <div className="flex-1 overflow-y-auto p-4 stack">
            {filteredConversations.length === 0 ? (
              <p className="small text-muted">
                {historySearch ? 'No conversations found' : 'No chat history yet'}
              </p>
            ) : (
      ...
      <div className="flex-1 max-w-7xl mx-auto h-screen flex flex-col w-full">
        <div className="p-4 cluster">
          <h1>Chat</h1>
        ...
        {error && <div className="px-4"><div className="alert alert--danger">{error}</div></div>}

        {/* Messages Scroll Area */}
        <div className="flex-1 overflow-y-auto px-4 pb-28 stack">
          {messages.map((message) => (
            <div key={message.id} className={`flex ${message.type === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`w-full max-w-6xl card ${message.type === 'user' ? 'btn--primary' : ''} ${message.type === 'assistant' ? 'relative' : ''}`}>
                {message.type === 'user' ? (
                  <div className="whitespace-pre-wrap">{message.content}</div>
                ) : (
                  <div>
                    {/* Copy button positioned at top-right - only for assistant messages */}
                    {message.type === 'assistant' && (
                      <button
                        onClick={() => handleCopyAsMarkdown(message.id, message.content)}
                    ...
                    <div 
                      ...
                      style={{
                        lineHeight: '1.6',
                        color: 'var(--text)',
                        paddingRight: message.type === 'assistant' ? '40px' : '0' // Make space for the button only on assistant messages
                      }}
